...
const ResponseSummaryPage = () => {
  ...
  const getChartType = (question) => {
    ...
    if (type === 'identification' || type === 'short-answer' || type === 'essay') return 'text-list';
  ...
  const generatePieChartData = (question) => {
    ...
    return {
      ...
      datasets: [{
        data: values,
        backgroundColor: resolvedLabels.map((_, i) => PALETTE[i % PALETTE.length]),
  ...
  const generateBarChartData = (question, horizontal = true) => {
    ...
    return {
      ...
      datasets: [{
        ...
        borderColor: backgroundColors.map(c => c.replace('0.7', '1').replace('0.65', '1')),
        borderWidth: 1,
        borderRadius: 6,
        borderSkipped: false,
      }]
  ...
  const barHorizontalOptions = {
    ...
    scales: {
      x: {
        ...
        grid: { color: isDarkMode ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.06)' },
      },
      y: {
        ticks: {
          color: THEME.muted,
    ...
    <div className="flex min-h-screen font-sans" style={{ backgroundColor: currentColors.background, color: currentColors.text }}>
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        >
          ...
          <div className="flex gap-0 mt-4 border-b" style={{ borderColor: currentColors.border }}>
            {[
              ...
              <button
                ...
                style={{
                  ...
                  color: activeView === tab.key ? "#3B82F6" : currentColors.textSecondary,
                ...
                onMouseEnter={e => { if (activeView !== tab.key) e.currentTarget.style.color = "#3B82F6"; }}
                onMouseLeave={e => { if (activeView !== tab.key) e.currentTarget.style.color = currentColors.textSecondary; }}
              >
                {tab.label}
              </button>
            ))}
          ...
          <div className="mb-8">
            ...
            <div className="space-y-6 sm:space-y-8">
              {responseSummaryData?.questionAnalysis?.map((question, index) => {
                ...
                const chartType = realQuestion ? getChartType(realQuestion) : 'pie';
                const chartData = realQuestion && chartType !== 'text-list' ? (
                ...
                const textAnswers = chartType === 'text-list' && realQuestion
                  ? realStudentData.map(student =>
                      student.answers?.[realQuestion.question_id] || student.answers?.[realQuestion.position]
                    ).filter(Boolean)
                  : [];
                ...
                const chartBadge = chartTypeLabels[chartType] || 'Response Distribution';
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        >
                          <div className="px-4 py-3 border-b" style={{ borderColor: currentColors.border }}>
                            <h4 className="text-xs font-semibold uppercase tracking-wider" style={{ color: currentColors.textSecondary }}>
                              {textAnswers.length} response{textAnswers.length !== 1 ? 's' : ''}
                            </h4>
                          </div>
                          <div className="divide-y max-h-64 overflow-y-auto" style={{ borderColor: currentColors.border }}>
                            {textAnswers.length > 0 ? textAnswers.map((ans, i) => (
                              <div key={i} className="px-4 py-3 text-sm" style={{ color: currentColors.text, borderColor: currentColors.border }}>
                                {ans}
                              </div>
                            )) : (
                              <div className="px-4 py-6 text-sm text-center" style={{ color: currentColors.textSecondary }}>No responses yet.</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-3 sm:w-64 flex-shrink-0">
                          <div className="rounded-xl border p-4" style={{ backgroundColor: isDarkMode ? '#1e2330' : '#F9FAFB', borderColor: currentColors.border }}>
                            <h4 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: currentColors.textSecondary }}>Statistics</h4>
                            <div className="space-y-2">
                              <div className="flex justify-between items-center gap-2">
                                <span className="text-sm" style={{ color: '#3B82F6' }}>Total Responses</span>
                                <span className="text-sm font-semibold" style={{ color: currentColors.text }}>{textAnswers.length}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        >
                          ...
                          <div className="p-4 flex flex-col sm:flex-row items-center gap-4">
                            ...
                            {chartType === 'bar-horizontal' && (
                              <div style={{ width: '100%', minWidth: 300, height: Math.max(100, chartData.labels.length * 52) }}>
                                <Bar data={chartData} options={barHorizontalOptions} />
                              </div>
                            )}
                                ...
                                <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                                  {chartData.labels.map((label, i) => (
                                    <div key={i} className="flex items-center gap-2 min-w-0">
                                      <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: chartData.datasets[0].backgroundColor[i] }} />
                                      <span className="text-xs break-words flex-1" style={{ color: currentColors.text }}>{label}</span>
                                      <span className="text-xs font-semibold flex-shrink-0" style={{ color: currentColors.textSecondary }}>{chartData.datasets[0].data[i]}</span>
                                    </div>
                                  ))}
                                </div>
                              </>
            ...
            <div>
              ...
              {responseSummaryData?.studentResults?.length > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        style={{ borderColor: currentColors.border, color: currentColors.text, backgroundColor: currentColors.surface }}
                      >
                        ← Prev
                      </button>
                      <span className="text-sm font-medium" style={{ color: currentColors.textSecondary }}>
                        {selectedStudentIndex + 1} / {responseSummaryData.studentResults.length}
                      </span>
                      <button
                        onClick={() => setSelectedStudentIndex(i => Math.min(responseSummaryData.studentResults.length - 1, i + 1))}
                        disabled={selectedStudentIndex === responseSummaryData.studentResults.length - 1}
                        className="px-3 py-1.5 rounded-lg text-sm font-medium border transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        style={{ borderColor: currentColors.border, color: currentColors.text, backgroundColor: currentColors.surface }}
                      >
                        Next →
                  ...
                  {/* Student info card */}
                  {(() => {
                    const student = responseSummaryData.studentResults[selectedStudentIndex];
                    return (
                      <div>
                        <div
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs" style={{ color: 'rgba(255,255,255,0.8)' }}>
                                Score: {student.score}% &nbsp;·&nbsp;
                                <span style={{ color: student.status === 'passed' ? '#86efac' : '#fca5a5' }}>
                        ...
                        <div className="space-y-4">
                          {realQuestions.map((question, qIdx) => {
                            ...
                            const isCorrect = checkIfAnswerIsCorrect(question, studentAnswer);
                            const correctAnswer = question.answers?.find(a => a.is_correct);
                            const correctLabel = correctAnswer?.letter_identifier || correctAnswer?.choice_answer || "N/A";
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start justify-between gap-3 mb-3">
                                  <p className="text-sm font-semibold flex-1" style={{ color: currentColors.text }}>
                                    Q{qIdx + 1}. {question.question}
                                  </p>
                                  <span
                                    className="text-xs px-2 py-0.5 rounded-full flex-shrink-0 font-semibold"
                                    style={{
                                      backgroundColor: isCorrect ? (isDarkMode ? '#052e16' : '#D1FAE5') : (isDarkMode ? '#2d0a0a' : '#FEE2E2'),
                                ...
                                <div className="flex flex-wrap gap-2 text-sm">
                                  ...
                                  {!isCorrect && (
                                    <span className="px-3 py-1 rounded-lg" style={{
                                      backgroundColor: isDarkMode ? '#052e16' : '#D1FAE5',
                                      color: '#10B981',
                                    }}>
                ...
                <div className="text-center py-16" style={{ color: currentColors.textSecondary }}>
