        ...
        return (
          <div key={rule.id} className="flex items-center gap-2 text-sm">
            {ok
              ...
              : <XCircle     size={15} className="flex-shrink-0" style={{ color: "#9ca3af" }} />}
            <span style={{ color: ok ? "#16a34a" : "#6b7280" }}>{rule.label}</span>
          </div>
        );
      })}
      {/* Strength bar */}
...
const validateStep2 = (role, fields) => {
  ...
  if (role === "student") {
    ...
    else if (!nameRegex.test(fields.studentFn.trim())) errors.studentFn = "Enter a valid first name";

    if (!fields.studentLn.trim()) errors.studentLn = "Last name is required";
    else if (!nameRegex.test(fields.studentLn.trim())) errors.studentLn = "Enter a valid last name";

    if (!fields.studentBd) errors.studentBd = "Birthdate is required";
    else {
      const age = Math.floor((Date.now() - new Date(fields.studentBd)) / (365.25 * 24 * 3600 * 1000));
      if (age < 10 || age > 100) errors.studentBd = "Enter a valid birthdate";
    }

    if (!fields.studentCourse) errors.studentCourse = "Please select a department";
    if (!fields.studentYrLvl)  errors.studentYrLvl  = "Please select a year level";
  ...
  if (role === "professor") {
    ...
    else if (!nameRegex.test(fields.profFn.trim())) errors.profFn = "Enter a valid first name";

    if (!fields.profLn.trim()) errors.profLn = "Last name is required";
    else if (!nameRegex.test(fields.profLn.trim())) errors.profLn = "Enter a valid last name";

    if (!fields.profDept) errors.profDept = "Please select a department";

    if (!fields.profBd) errors.profBd = "Birthdate is required";
    else {
      const age = Math.floor((Date.now() - new Date(fields.profBd)) / (365.25 * 24 * 3600 * 1000));
...
const OnBoarding = () => {
  ...
  const confirmMatchState = confirmTouched && passwordConfirm
    ? password === passwordConfirm ? "match" : "mismatch"
    : null;
              ...
              <div>
                ...
                {confirmMatchState === "match" && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs" style={{ color: "#16a34a" }}>Passwords match</span>
                  </div>
                )}
                {confirmMatchState === "mismatch" && (
                  <div className="flex items-center gap-1 mt-1">
                    <XCircle size={13} style={{ color: "#ef4444" }} />
                    <span className="text-xs" style={{ color: "#ef4444" }}>Passwords do not match</span>
                  </div>
                )}
              </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <InputField type="text" placeholder="First Name" value={studentFn}
                        onChange={(e) => { setStudentFn(e.target.value); setFieldErrors((p) => ({ ...p, studentFn: "" })); }}
                    ...
                    <div>
                      ...
                      <InputField type="text" placeholder="Last Name" value={studentLn}
                        onChange={(e) => { setStudentLn(e.target.value); setFieldErrors((p) => ({ ...p, studentLn: "" })); }}
                  ...
                  <div>
                    ...
                    <InputField type="date" placeholder="Birthdate" value={studentBd}
                      onChange={(e) => { setStudentBd(e.target.value); setFieldErrors((p) => ({ ...p, studentBd: "" })); }}
                      ...
                      {showDeptDropdown && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-black rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                          {departmentOptions.map((dept) => (
                            <button key={dept.code} type="button"
                              ...
                              onClick={() => { setStudentCourse(dept.code); setShowDeptDropdown(false); setFieldErrors((p) => ({ ...p, studentCourse: "" })); }}
                            >
                  ...
                  <div>
                    <select
                      ...
                      value={studentYrLvl}
                      onChange={(e) => { setStudentYrLvl(e.target.value); setFieldErrors((p) => ({ ...p, studentYrLvl: "" })); }}
                    >
                      <option value="" className="text-black">Select Year Level</option>
                      {yearLevelOptions.map((yearLevel) => (
                        <option key={yearLevel.code} value={yearLevel.code} className="text-black">{yearLevel.name}</option>
                      ))}
                    </select>
                  ...
                  <div>
                    <select
                      ...
                      value={studentGender}
                      onChange={(e) => { setStudentGender(e.target.value); setFieldErrors((p) => ({ ...p, studentGender: "" })); }}
                    >
                      <option value="" className="text-black">Select Gender</option>
                      {genderOptions.map((gender) => (
                        <option key={gender.code} value={gender.code} className="text-black">{gender.name}</option>
                      ))}
                    </select>
              ...
              {role === "professor" && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">First Name</label>
                      <InputField type="text" placeholder="First Name" value={profFn}
                        onChange={(e) => { setProfFn(e.target.value); setFieldErrors((p) => ({ ...p, profFn: "" })); }}
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Last Name</label>
                      <InputField type="text" placeholder="Last Name" value={profLn}
                        onChange={(e) => { setProfLn(e.target.value); setFieldErrors((p) => ({ ...p, profLn: "" })); }}
                      ...
                      {showDeptDropdown && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-black rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                          {departmentOptions.map((dept) => (
                            <button key={dept.code} type="button"
                              className="w-full px-4 py-2 text-left text-gray-900 hover:bg-gray-100 transition border-b border-gray-200 last:border-b-0 bg-white"
                              onClick={() => { setProfDept(dept.code); setShowDeptDropdown(false); setFieldErrors((p) => ({ ...p, profDept: "" })); }}
                            >
                              <span className="font-medium">{dept.code}:</span> {dept.name}
                            </button>
                          ))}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Birthdate</label>
                    <InputField type="date" placeholder="Birthdate" value={profBd}
                      onChange={(e) => { setProfBd(e.target.value); setFieldErrors((p) => ({ ...p, profBd: "" })); }}
                  ...
                  <div>
                    <select
                      ...
                      value={profGender}
                      onChange={(e) => { setProfGender(e.target.value); setFieldErrors((p) => ({ ...p, profGender: "" })); }}
                    >
                      <option value="" className="text-black">Select Gender</option>
                      {genderOptions.map((gender) => (
                        <option key={gender.code} value={gender.code} className="text-black">{gender.name}</option>
