...
const ViewAllTaskPage = () => {
  ...
  const { studentResponseData, studentResponseDataLoading, setTaskId } =
    useSpace();
  ...
  const individualTasks = sortedTasks.filter(
    (task) => task.task_category === "individual-activity",
  );
  const examTasks = sortedTasks.filter((task) => task.task_category === "exam");
  ...
  const handleViewScore = (task) => {
    ...
    if (!studentResponseData?.success) {
      ...
      toast.error(studentResponseData?.message);
      return;
    }
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-10 pt-20 sm:pt-24 lg:pt-10 overflow-y-auto">
          ...
          {!isLoading && !error && (
            <>
              ...
              {/* Tasks Table */}
              <div
                className="p-3 sm:p-4 lg:p-6 xl:p-8 rounded-2xl shadow-lg max-w-7xl mx-auto"
                style={{
                  backgroundColor: currentColors.surface,
                ...
                {quizTasks.length > 0 && (
                  <div className="mb-8">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      >
                        {quizTasks.length}{" "}
                        {quizTasks.length === 1 ? "quiz" : "quizzes"}
                      </span>
                    </div>
                          ...
                          {quizTasks.map((task) => (
                            ...
                            >
                              <td className="py-4 px-4">
                                <span
                                  className={`inline-block px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(task)}`}
                                >
                                  {task.has_answered
                                    ? "Completed"
                        ...
                        >
                          <div className="flex flex-col space-y-3">
                            ...
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className="inline-block px-3 py-1 rounded-full text-xs font-medium border w-fit"
                                style={{
                                  backgroundColor: getStatusColor(
                                    ...
                                    ? isDarkMode
                                      ? "rgba(16, 185, 100, 0.2)"
                                      : "rgba(34, 197, 94, 0.2)"
                                    : getStatusColor(task).includes("red")
                                      ? isDarkMode
                                        ? "rgba(239, 68, 68, 0.2)"
                                        : "rgba(239, 68, 68, 0.2)"
                                      : isDarkMode
                                        ? "rgba(59, 130, 246, 0.2)"
                                        : "rgba(59, 130, 246, 0.2)",
                                  color: getStatusColor(task).includes("green")
                                    ? isDarkMode
                                      ? "#10e164"
                                      : "#22c55e"
                                    : getStatusColor(task).includes("red")
                                      ...
                                      : isDarkMode
                                        ? "#4d9bef"
                                        : "#3b82f6",
                                  borderColor: getStatusColor(task).includes(
                                    "green",
                                  )
                                    ? isDarkMode
                                      ? "#00b865"
                ...
                {examTasks.length > 0 && (
                  <div className="mb-8">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      >
                        {examTasks.length}{" "}
                        {examTasks.length === 1 ? "exam" : "exams"}
                      </span>
                    </div>

                    {/* Desktop Table for Exams */}
                    <div
                      className="hidden lg:block overflow-x-auto rounded-xl border"
                      style={{
                        backgroundColor: currentColors.surface,
                ...
                {groupTasks.length > 0 && (
                  <div className="mb-8">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      >
                        {groupTasks.length}{" "}
                        {groupTasks.length === 1 ? "activity" : "activities"}
                      </span>
                    </div>
                    ...
                    {question.type === "mcq" && (
                      <div className="space-y-2">
                        {question.answers?.map((answer) => {
                          ...
                          const isSelected =
                            answer.letter_identifier === studentAnswer;
