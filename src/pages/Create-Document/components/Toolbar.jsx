...
const Toolbar = ({
  ...
  const handleDropdownToggle = (name, open) => {
    ...
    switch (name) {
      ...
      case "image":     setIsImageDropdownOpen(true);     break;
      case "paperSize": setIsPaperSizeDropdownOpen(true); break;
  ...
  const fontOptions = [
    "Inter","Arial","Times New Roman","Calibri","Verdana",
    "Georgia","Courier New","Helvetica","Tahoma","Trebuchet MS",
  ];

  const fontSizeOptions = [10, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72];

  const colorOptions = [
    "default","black","white","green","red","blue","gold","orange",
    "purple","pink","brown","teal","navy","gray",
  ];
  ...
  const applyFormatting = (command, value = null) => {
    ...
    if (command === "fontSize") {
      ...
      if (selection?.rangeCount > 0) {
        ...
        } catch { document.execCommand(command, false, value); }
      } else {
  ...
  const handleMouseDown = (e, cb) => { e.preventDefault(); cb(); };
  ...
  const applyTextColor = (color) => {
    const actual = color === "default" ? (isDarkMode ? "white" : "black") : color;
  ...
  const applyList = (listType, style = null) => {
    ...
    setTimeout(() => { editorRef.current?.focus(); setIsListDropdownOpen(false); }, 10);
  };
  ...
  const hoverOn  = (e) => { e.currentTarget.style.backgroundColor = currentColors.background; };
  ...
  const Divider = () => (
    <div className="h-6 w-px flex-shrink-0" style={{ backgroundColor: currentColors.border }} />
  );
  ...
  return (
    ...
    <div
      ...
      style={{ backgroundColor: currentColors.background, borderColor: currentColors.border, zIndex: 20 }}
    >
      <div className="flex flex-nowrap items-center justify-center gap-2 px-4 py-2 min-w-max mx-auto">
        ...
        <div className="relative flex-shrink-0">
          ...
          >
            <span className="text-sm font-medium min-w-[28px] text-center">{selectedFontSize}</span>
            <FiChevronDown size={CHEVRON} />
          </div>

          {isFontSizeDropdownOpen && (
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 w-44 max-h-52 overflow-y-auto" style={dropdownStyle}>
              {fontSizeOptions.map((size) => (
                <div
                  key={size}
                  className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded"
                  style={{ color: currentColors.text }}
                  onMouseDown={(e) => handleMouseDown(e, () => applyFontSize(size))}
                  onMouseEnter={hoverOn}
                  onMouseLeave={hoverOff}
                >
                  <span style={{ fontSize: `${Math.min(Math.max(size, 12), 20)}px`, minWidth: 16 }}>T</span>
                  <span className="text-sm" style={{ color: currentColors.textSecondary }}>{size}px</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="relative flex-shrink-0">
          ...
          {isAlignmentDropdownOpen && (
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 min-w-[140px]" style={dropdownStyle}>
              {[
                ...
                <div
                  ...
                  className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded"
                  style={{ color: currentColors.text }}
                  onMouseDown={(e) => handleMouseDown(e, () => applyAlignment(align))}
                  onMouseEnter={hoverOn}
                  onMouseLeave={hoverOff}
                >
                  {icon}
        ...
        <div className="relative flex-shrink-0">
          ...
          {isColorDropdownOpen && (
            ...
            >
              <div className="text-xs font-semibold mb-3" style={{ color: currentColors.textSecondary }}>
                Text Color
              </div>
              <div className="grid grid-cols-4 gap-4">
                {colorOptions.map((color) => (
                  <button
                    ...
                    onMouseDown={(e) => handleMouseDown(e, () => applyTextColor(color))}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="relative flex-shrink-0">
          ...
          {isImageDropdownOpen && (
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 min-w-[120px]" style={dropdownStyle}>
              {[["crop", <FiCrop size={14} />, "Crop"], ["rotate", <FiRotateCw size={14} />, "Rotate"]].map(
                ([action, icon, label]) => (
                  <div
                    key={action}
                    className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded"
                    style={{ color: currentColors.text }}
                    onMouseDown={(e) => handleMouseDown(e, () => handleImageAction(action))}
                    onMouseEnter={hoverOn}
                    onMouseLeave={hoverOff}
                  >
                    {icon}
        ...
        <div className="relative flex-shrink-0">
          ...
          {isPaperSizeDropdownOpen && (
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 w-48" style={dropdownStyle}>
              {Object.entries(paperSizes).map(([name, { width, height }]) => (
                <div
                  ...
                  style={{ color: currentColors.text }}
                  onMouseDown={(e) => handleMouseDown(e, () => applyPaperSize(name))}
                  onMouseEnter={hoverOn}
                  onMouseLeave={hoverOff}
                >
            ...
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 w-48" style={dropdownStyle}>
              {fontOptions.map((font) => (
                <div
                  ...
                  style={{ color: currentColors.text }}
                  onMouseDown={(e) => handleMouseDown(e, () => applyFontFamily(font))}
                  onMouseEnter={hoverOn}
                  onMouseLeave={hoverOff}
                >
        ...
        <div className="relative flex-shrink-0">
          ...
          {isListDropdownOpen && (
            <div className="absolute top-full mt-1 rounded shadow-lg z-10 w-52" style={dropdownStyle}>
              <div className="border-b pb-1 mb-1" style={{ borderColor: currentColors.border }}>
                <div className="px-3 py-1 text-xs font-semibold" style={{ color: currentColors.textSecondary }}>
                  BULLETED
                </div>
                <div
                  className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded"
                  style={{ color: currentColors.text }}
                  onMouseDown={(e) => handleMouseDown(e, () => applyList("bullet"))}
                  onMouseEnter={hoverOn}
                  onMouseLeave={hoverOff}
                >
              ...
              <div>
                ...
                {[
                  ...
                  <div
                    ...
                    className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded"
                    style={{ color: currentColors.text }}
                    onMouseDown={(e) => handleMouseDown(e, () => applyList("number", style))}
                    onMouseEnter={hoverOn}
                    onMouseLeave={hoverOff}
                  >
                    <span className="text-sm">{label}</span>
