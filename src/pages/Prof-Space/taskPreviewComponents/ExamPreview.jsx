  ...
  const getQuestions = () => {
    ...
    return taskData.questions.map((questionObj, index) => {
      ...
      return {
        ...
        points: Math.floor((taskData?.total_score || 100) / (taskData?.questions?.length || 1))
      };
    });
  };
            ...
            {question.answers.map((answer, index) => (
              <div 
                ...
                style={{
                  ...
                  borderWidth: hoveredOption === `${question.id}-${answer.letter_identifier}` ? '2px' : '1px',
                  borderColor: hoveredOption === `${question.id}-${answer.letter_identifier}` 
                    ? '#2563eb' 
                    : currentColors.border,
                }}
                onMouseEnter={() => setHoveredOption(`${question.id}-${answer.letter_identifier}`)}
                  ...
                  <div
                    className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-semibold transition-all ${
                      userAnswers[question.id] === answer.letter_identifier
                        ...
                        : hoveredOption === `${question.id}-${answer.letter_identifier}`
                        ? 'border-blue-400 text-blue-600'
                        : 'border-gray-300 text-gray-500'
                    }`}
                    style={{
                      borderColor: userAnswers[question.id] === answer.letter_identifier
                        ? '#2563eb'
                        : hoveredOption === `${question.id}-${answer.letter_identifier}`
                        ? '#2563eb'
                        : currentColors.border,
                      backgroundColor: userAnswers[question.id] === answer.letter_identifier
                        ? '#2563eb'
                        : currentColors.background,
                      color: userAnswers[question.id] === answer.letter_identifier
                        ? '#ffffff'
                        : hoveredOption === `${question.id}-${answer.letter_identifier}`
                        ? '#2563eb'
                        : currentColors.text,
    ...
    <div className="max-w-4xl mx-auto p-6" style={{ backgroundColor: currentColors.background }}>
      ...
      <div className="mb-8 p-6 rounded-lg border" style={{
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                <span className="font-semibold" style={{ color: currentColors.textSecondary }}>Questions:</span>
                <p style={{ color: currentColors.text }}>{questions.length}</p>
              </div>
              <div>
                <span className="font-semibold" style={{ color: currentColors.textSecondary }}>Duration:</span>
          ...
          <div className="text-center p-4 rounded-lg" style={{
            ...
            <p className="text-sm font-semibold text-red-800 mb-1">Time Remaining</p>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-1">
            ...
            <span style={{ color: currentColors.text }}>{answeredQuestions}/{questions.length} answered</span>
          </div>
      ...
      <div className="space-y-8">
        {questions.map((question, index) => (
          <div key={question.id} className="p-6 rounded-lg border" style={{
            backgroundColor: currentColors.surface,
            borderColor: currentColors.border
          }}>
            {/* Question Header */}
            ...
            {/* Question Text */}
            <p className="text-lg leading-relaxed mb-4" style={{ color: currentColors.text }}>
              {question.question}
            </p>
      ...
      <div className="mt-8 p-6 rounded-lg border" style={{
        ...
        <div className="flex justify-between items-center">
          <div>
            <p className="font-semibold mb-2" style={{ color: currentColors.text }}>
              Exam Summary
            </p>
      ...
      <div className="mt-4 p-4 rounded-lg" style={{
        ...
        <p className="text-sm text-amber-800">
          <strong>Warning:</strong> This is a preview of the exam. The actual exam will be timed and submissions cannot be modified after submission.
        </p>
      </div>
    </div>
  );
